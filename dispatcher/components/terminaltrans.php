<?php
		...
		$alltrans = mysqli_query($link, "SELECT * FROM ".DB_PREFIX."terminaltrans WHERE terminal_ID = '".$Ter_ID."' ORDER BY ID DESC"); 
		$wallet = mysqli_query($link, "SELECT A.*, B.dispatcher_ID FROM ".DB_PREFIX."terminaltrans A, ".DB_PREFIX."terminalload_wallet B WHERE A.terminal_ID = '".$Ter_ID."' AND A.transaction_code = 'RLD' AND B.transaction_no=A.ref_no ORDER BY ID DESC"); 
		$collect = mysqli_query($link, "SELECT * FROM ".DB_PREFIX."terminaltrans WHERE terminal_ID = '".$Ter_ID."' AND `transaction_code` = 'SF' ORDER BY ID DESC"); 
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            ...
                            <ul class="nav nav-tabs">
								...
								<li><a href="#loadwallet" data-toggle="tab">Load Wallet</a></li>
								<li><a href="#collection" data-toggle="tab">Fare Collection</a></li>
					...
					<!-- Load Wallet -->
								<div class="tab-pane fade in active" id="loadwallet">
									<div class="panel-body">
										<div class="dataTable_wrapper">
											<table class="table table-striped table-bordered table-hover" id="dataTables-terminaltransacations">
												<thead>
												...
												<tbody>
												<?php 
													WHILE($trans = mysqli_fetch_array($wallet)){
													$tcode = $trans['transaction_code'];
													$transcodes = mysqli_fetch_array(mysqli_query($link, "SELECT description FROM ".DB_PREFIX."transactioncode WHERE transaction_code = '".$tcode."'"));
													$passenger = mysqli_fetch_array(mysqli_query($link, "SELECT description FROM ".DB_PREFIX."transactioncode WHERE transaction_code = '".$tcode."'"));
													$dID = $trans['dispatcher_ID'];
													$dname = mysqli_fetch_array(mysqli_query($link, "SELECT fname, lname, suffix FROM ".DB_PREFIX."users WHERE user_ID = '{$dID}'"));
													$dispatchername = STRTOUPPER(substr($dname['fname'],0,1).". ".$dname['lname'].(($dname['suffix'])? ", ".$dname['suffix'] : ""));
												?>
												<tr class="gradeA">
													<td><?= $transcodes['description']; ?></td>
													<td><?= $trans['trans_date']." ".$trans['trans_time']; ?></td>
													<td><?= $trans['ref_no']; ?></td>
													<td class="center"><?= $trans['user_ID']; ?></td>
													<td class="center"><?= $dispatchername ?></td>
									...
									<div class="panel-body">
										<div class="dataTable_wrapper">
											<table class="table table-striped table-bordered table-hover" id="dataTables-terminaltransacations">
												...
												<tbody>
												<?php 
													WHILE($trans = mysqli_fetch_array($collect)){
													$tcode = $trans['transaction_code'];
													$transcodes = mysqli_fetch_array(mysqli_query($link, "SELECT description FROM ".DB_PREFIX."transactioncode WHERE transaction_code = '".$tcode."'"));
													$passenger = mysqli_fetch_array(mysqli_query($link, "SELECT description FROM ".DB_PREFIX."transactioncode WHERE transaction_code = '".$tcode."'"));
												?>
												<tr class="gradeA">
													...
													<td><?= $trans['ref_no']; ?></td>
													<td class="center"><?= $trans['user_ID'] ?></td>
													<td class="right"><?= $trans['service_fee'] ?></td>
												</tr>
												<?php } ?>
												</tbody>
											</table>
