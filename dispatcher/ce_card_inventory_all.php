											...
											<div class="dataTable_wrapper">
												<table class="table table-striped table-bordered table-hover" id="dataTables-payout">
													...
													<?php
														...
														$query_cards = mysqli_query($link, "SELECT card_number, status, disposed FROM ".DB_PREFIX."idcards WHERE terminal_ID='{$terminal_ID}'");
														
														WHILE($id_card = mysqli_fetch_array($query_cards)){
															$status = $id_card['status'];
															$card = $id_card['card_number'];
															$disposed = $id_card['disposed'];
														?>
														<tr class="gradeA">
															<td><a href="gen_id_bridge?terminal=<?= $card; ?>" data-toggle="tooltip" title="View Terminal Transactions"><?= $card; ?></a></td>
															<td><span style="float:left;color:#0000FF;"></span><?= ($disposed == 0)? "Available" : "Sold"; ?></td>
														</tr>
													<?php } ?>
													</tbody>
												</table>											
