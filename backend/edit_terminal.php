                            ...
                            ELSE:
                                ...
                                ?>
                                    ...
                                    <div class="container" id="genpro" style="display:block;">								
										...
										<div class="alert alert-success" style="width:50%;" id="dlink-display">
											<div class="12u(xsmall) pull-left" >

													<a href="javascript: null(void)" data-toggle="modal" data-target="#did-edit"><i class="fa fa-plus"></i> Add Terminal Dispatchers</a>
											
											</div>
											<hr/>
											<?php
												$count = "";
												$dispatcher_info = mysqli_query($link, "SELECT email, fname, mname, lname, suffix, user_ID, account_status  FROM " . DB_PREFIX . "users WHERE terminal_ID = '{$terminal_ID}'");
												$total = mysqli_num_rows($dispatcher_info);
												WHILE($dispatcher=mysqli_fetch_array($dispatcher_info)){
												
												$account_status = $dispatcher['account_status'];
												$dispatcherID = $dispatcher['user_ID'];
												
												IF(strpos($dispatcherID, 'TEMP') !== false):
												...
												$count+=1;
											?>
											<div class="12u(xsmall) pull-left" id="did-display">
												<div class="12u(xsmall) pull-left" style=" width:180px;">Dispatcher <?php IF ($total>1):echo $count; ENDIF; ?> ID:</div>
												<div class="6u(xsmall) pull-left" id="did-result">
													<span class="did"><strong><a href="gen_id_bridge?memberID=<?= $dispatcherID ?>"><?= $dispatcherID ?></a></strong></span>
												</div>
											</div>
											<div style="clear:both"></div>	
											<div class="12u(xsmall) pull-left" id="dname-display">
												<div class="12u(xsmall) pull-left" style=" width:180px;">Dispatcher <?php IF ($total>1): echo $count; ENDIF; ?> Name:</div>
												<div class="6u(xsmall) pull-left">
													<strong>
														<span class="fname"><?= $dispatcher['fname']; ?></span>
															<span class="lname"><?= $dispatcher['lname']; ?></span>
															<?php
															IF ($dispatcher['suffix'] != ""): echo ", " . $dispatcher['suffix'];
															ENDIF;
															?>
													</strong> - <?=$dstatus;?>
												 </div>
											</div>
											<span class="pull-right"><a href="javascript: null(void)" data-toggle="modal" data-target="#delete<?=$dispatcherID?>" class="btn btn-danger btn-circle" style="width:20px;height:20px;padding:1px 0"><i class="fa fa-times"></i></a></span>
											<hr/>
											<div class="modal fade" id="delete<?=$dispatcherID?>" tabindex="-1" role="dialog" aria-labelledby="DeleteDIDmod" aria-hidden="true">
												<div class="modal-dialog">
													<div class="modal-content">
														<div class="modal-header">
															<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
															<h4 class="modal-title" id="DeleteDIDmod">Delete Dispatcher</h4>
														</div>
														<div class="modal-body">
															<strong>
																<span class="fname"><?= $dispatcher['fname']; ?></span>
														...
														<div class="modal-footer">
															<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
															<a href="bridge?dispatcherID=<?=$dispatcherID?>&TID=<?=$terminal_ID?>" class="btn btn-danger" id="<?=$dispatcherID?>">Remove Dispatcher</a>
														</div>
													</div>
												</div>
											</div>
                                        ...
                                        <div class="modal fade" id="did-edit" tabindex="-1" role="dialog" aria-labelledby="Editdidmod" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                                        <h4 class="modal-title" id="Editdidmod">Add Dispatcher</h4>
                                                    </div>
                                                    <div class="modal-body">
                                                        <div class="form-group has-success" id="dispatcher-results"></div>
														<div class="form-group has-success" id="dispatcher-field">
                                                            <input class="form-control ndid" type="text" name="did" id="did" value="" PLACEHOLDER="Dispatcher ID" onkeypress="return isNumberKey(event)"/>
															<input class="form-control" type="hidden" name="tid" id="tid" value="<?=$terminal_ID?>"/>
                                                        </div>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                                                        <button type="button" class="btn btn-primary" id="didsubmit">Save changes</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
        ...
        <script language="javascript" type="text/javascript">
			...
			$(document).ready(function() {
				...
				$("#didsubmit").click(function() {
					...
					$.post("trigger/updateterminalform", {
						...
						function(data) {
							...
							$('#dispatcher-results').delay().fadeIn();
							//$('#dispatcher-results').delay(3000).fadeOut();
						}
					);
					//$(".did").text($("#did").val());
				...
				$("#did").blur(function(e) {
					...
					if (did.length < 10) {
						$("#dispatcher-results").html('<img src="../myaccount/images/not-available.png" />ID no. must be at least 10 characters');
						return;
					} else if (did.length >= 10) {
						$("#dispatcher-results").html('<img src="../myaccount/images/loading.gif" />');
						$.post('trigger/exec', {
							'did': did, 'tid': tid
						}, function(data) {
							$("#dispatcher-results").html(data);
