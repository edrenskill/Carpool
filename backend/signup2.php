<?php 
	...
	$step = $_SESSION['step'];
	IF(isset($step) && $step == 2 && isset($$_SESSION['dact_no'])): header('location: add_vehicle'); ENDIF;
	$member_type = $_SESSION['application_type'];
	IF($member_type == 1 || $member_type == 7 || $member_type == 2):
		$location = "confirmation";
		
	ELSEIF($member_type == 10 || $member_type == 8):
		IF($step == 1 && $member_type == 8):
			$location = "driver_signup";
		ELSEIF(($step == 1 && $member_type == 10) || $step == 2):
			$location = "add_vehicle";
		ENDIF;
	ENDIF;
	...
	IF(isset($_POST) && array_key_exists('doRegister',$_POST)):
		...
		IF($member_type == 7): 
			...
			ELSE:
				...
				$results = mysqli_query($link, "SELECT ID, driver_ID1, driver_ID2 FROM ".DB_PREFIX."vehicles WHERE unit_ID='{$vID}'");
				//return total count
				$unit_exist = mysqli_num_rows($results); //total records
				
				IF(!$unit_exist) :
					$err[] = "Vehicle ID not found!";
				ELSE:

					$check_vehicle = mysqli_fetch_array($results);
					$slot1 = $check_vehicle['driver_ID1'];
					$slot2 = $check_vehicle['driver_ID2'];
					
					IF(!isset($slot1) || $slot1==""): 
						$set = "SET driver_ID1";
					ELSE:
						IF(!isset($slot2) || $slot2==""): 
							$set = "SET driver_ID2";
						ELSE:
							$err[] = "Please remove at least 1 driver from unit with ID #:<strong>".$vID."</strong>";
						ENDIF;	
					ENDIF;
				ENDIF;
			ENDIF; 
		...
		IF($member_type == 7 || $member_type == 10):
		...
		IF(!empty($err)):
			...
			// Driver only
			IF($member_type == 7):
				$_SESSION['post']['vehicle_ID'] = $data['vehicle_ID'];
			ENDIF;
			
			// Driver and Operator/Driver
			IF($member_type == 7 || $member_type == 10):
		...
		ELSEIF(empty($err)):
			...
			VALUES ('$username','$sha1pass','$member_type','$email','$fname','$mname','$lname','$suffix','$gender','$dob','$address','$country','$region','$province','$city','$barangay','$contact',now(),'$userip',0,'$activcode','$gen_account','$member_batch')";
			...
			// INSERT DRIVER FOR VEHICLE ID
			IF($member_type == 7): 
				mysqli_query($link, "UPDATE `".DB_PREFIX."vehicles` {$set}='{$gen_account}' WHERE unit_ID='{$vID}'");
			ENDIF;
			
			// INSERT OPERATOR / DRIVER
			IF($member_type == 7 || $member_type == 10): 
...
<html>
	...
	// FORM VALIDATION
		function validateForm()
		{
			...
			<?php IF ($member_type == 7 || $member_type == 10): ?>
...
//			if (email==null || email=="") { alert("Enter valid email"); return false; }
			...
			<?php IF ($member_type == 7 || $member_type == 10): ?>
				if (nbi==null || nbi=="") { alert("Please e nter NBI clearance number"); return false; }
				if (nbi_expire==null || nbi_expire=="") { alert("Enter NBI Clearance expi5r6ation date"); return false; }
				if (dl==null || dl=="") { alert("Enter Driver's license number"); return false; }
				if (dl_expire==null || dl_expire=="") { alert("Enter Driver's licence expiration date"); return false; }
				if (police==null || police=="") { alert("Enter Police Clearance number"); return false; }
				if (police_expire==null || police_expire=="") { alert("Enter Police clearance expiration date"); return false; }
			<?php ENDIF; ?>
			...
			$(document).ready(function() {
				...
				<?php IF($member_type == 7): ?>
				// IF VEHICLE ID ENTERED
				$("#vehicle_ID").blur(function (e) {
					//removes spaces from email
					$(this).val($(this).val().replace(/\s/g, ''));
					var vID = $(this).val();
					
					if(vID.length < 1){$("#vID-result").html('');return;}

					if(vID.length >= 1){
						$("#vID-result").html('<img src="../myaccount/images/loading.gif" />');
						$.post('../myaccount/includes/validation/validate', {'vID':vID}, function(data) {
							$("#vID-result").html(data);
						});
					}
				});
				<?php ENDIF; ?>
	...
	<body>
		...
		<div id="wrapper">
			...
			<div id="page-wrapper">
				<div class="row">
					<div class="col-lg-12">
						<h3 class="page-header">Membership Form for <span class="text-success"><?php IF(isset($_SESSION['application_type'])): ?> 
							<?php
								$Mtype = $_SESSION['application_type'];
								IF($Mtype == 1):
									echo "Commuter";
								ELSEIF($Mtype == 7):
									echo "Driver";
						...
						<?php // Display error message
							IF(!empty($err)) : 
								echo "<div class=\"alert alert-danger\"><strong>Please check the following fields:</strong><br />"; FOREACH ($err as $e) { echo "$e <br />"; } 
									...
									IF($member_type == 7):
										$field_vID = @$_SESSION['post']['vehicle_ID'];
									ENDIF;
								...
								<section id="signup">
									<div class="container">
										<span style="color:#003399">Note: <b>Field with asterisk (<span style="color:#FF0000">*</span>) are required.</b></span><br /><br />
											<form role="form" name="enroll" id="enroll" method="post" action="signup2" onsubmit="return validateForm()" autocomplete="off">
												...
												<div class="form-group">
													...
													<input class="form-control" type="text" name="vehicle_ID" id="vehicle_ID" value="<?php IF(isset($field_vID)): echo $field_vID; ENDIF; ?>" PLACEHOLDER="Vehicle ID" style="width:300px"/><span id="vID-result" style="padding-left:10px"></span>
												</div>
												<?php ENDIF; ?>
