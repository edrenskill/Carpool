<?php 
	...
	$member_batch = 0;
	(!isset($_SESSION['member_batch']))? $member_batch = 0 : $member_batch = $_SESSION['member_batch']; 
	$batchselect = mysqli_fetch_array(mysqli_query($link, "SELECT  `ID`, `batch_name`, `no_of_members` FROM `".DB_PREFIX."member_batch` WHERE `ID` = {$member_batch}"));	
	...
	IF(isset($_POST) && array_key_exists('doRegister',$_POST)):
		...
		IF(!isset($data['membertype']) || $data['membertype'] == ""): $err[] = "Membership Type"; ENDIF;
		IF(!isset($data['IDCardtype']) || $data['IDCardtype'] == ""): $err[] = "ID Card Type"; ENDIF;
...
//		IF($data['contact'] == "" || $data['contact'] == 0): $err[] = "Contact Number"; ENDIF;
		...
		IF(!empty($err)):
			$_SESSION['post']['membertype'] = $data['membertype'];
			$_SESSION['post']['IDCardtype'] = $data['IDCardtype'];
		...
		ELSEIF(empty($err)):
			...
			$cfname = STRTOUPPER($data['cfname']." ".$data['clname']);
			...
			ELSEIF($IDCardtype == 2):
				//SET TO AUTOMATIC
				$gen_account = $batch.mysqli_real_escape_string($link, GenID());
			...
			VALUES ('$username','$sha1pass','$membertype','$email','$fname','$mname','$lname','$suffix','$gender','$dob','$address','$country','$region','$province','$city','$barangay','$contact',now(),'$userip',0,'$activcode','$gen_account','$member_batch')";
			...
			$total_passenger = $batchselect['no_of_members']+1;
			$BatchID = $batchselect['ID'];
			
			// Update user md5
			mysqli_query($link, "UPDATE `".DB_PREFIX."users` SET md5_id='$md5_id' WHERE id='$user_id'");
			mysqli_query($link, "UPDATE `".DB_PREFIX."member_batch` SET no_of_members='$total_passenger' WHERE ID='$BatchID'");
...
<html>
	...
	function validateForm()
		{
			var membertype = document.forms['enroll'].elements['membertype'];
			var IDCardtype = document.forms['enroll'].elements['IDCardtype'];
			...
			if(chkvaluem==''){ alert('Membership type must be selected.'); return false; }
			
			len=IDCardtype.length-1;
			chkvaluec='';
			for(i=0; i<=len; i++){ if(IDCardtype[i].checked)chkvaluec=IDCardtype[i].value; }
			if(chkvaluec==''){ alert('ID Card type must be selected.'); return false; }
...
/*				$("#email").blur(function (e) {
					...
					if(email.length >= 1){
						...
						$.post('../includes/validation/validate', {'email':email}, function(data) {
			...
			<div id="wrapper">
				...
				<!-- Main -->
					...
					IF(!empty($err)) : 
						...
						$field_member = @$_SESSION['post']['membertype'];
						$field_IDCard = $_SESSION['post']['IDCardtype'];
					...
					?>
						...
						<!-- SIGN UP -->
							<section id="signup">
								<div class="container">
									<h2>Membership Form <?php IF(isset($batchselect['batch_name'])): ?>- Batch <?php strtoupper($batchselect['batch_name']); ENDIF; ?></h2>
									<span style="color:#003399">Note: <b>Field with asterisk (<span style="color:#FF0000">*</span>) are required.</b></span><br /><br />
										<form role="form" name="enroll" id="enroll" method="post" action="signup" onsubmit="return validateForm()">
											...
											<div class="form-group">
												<label>ID Card Type<span style="color:#FF0000">*</span></label>
                                                <div class="radio">
                                                    <label>
														<input class="radiobox" type="radio" name="IDCardtype" id="IDCardtype1" value="1" <?php IF(isset($field_IDCard) && $field_IDCard == '1'): echo 'checked="checked"'; ENDIF; ?> />RFID Type
                                                    </label>
                                                </div>
                                                <div class="radio">
                                                    <label>
                                                        <input class="radiobox" type="radio" name="IDCardtype" id="IDCardtype2" value="2" <?php IF(isset($field_member) && $field_member == '2'): echo 'checked="checked"'; ENDIF; ?> />Ordinay PVC
                                                    </label>
                                                </div>
											</div>
											...
											<div class="form-group">
												...
												<div>
													<input class="form-control" type="text" name="fname" id="fname" value="<?php IF(isset($field_fname)): echo $field_fname; ENDIF; ?>" PLACEHOLDER="First Name" style="float:left; margin-right:20px;width:180px"/>
													<input class="form-control" type="text" name="mname" id="mname" value="<?php IF(isset($field_mname)): echo $field_mname; ENDIF; ?>" PLACEHOLDER="Middle Name" style="float:left; margin-right:20px;width:180px"/>
													<input class="form-control" type="text" name="lname" id="lname" value="<?php IF(isset($field_lname)): echo $field_lname; ENDIF; ?>" PLACEHOLDER="Last Name" style="float:left; margin-right:20px;width:180px"/>
											...
											<div class="form-group">											
												...
												<input class="form-control" type="date" name="dob" id="dob" PLACEHOLDER="mm/dd/YYYY" value="<?php IF(isset($field_dob)): echo $field_dob; ENDIF; ?>" onchange="submitBday()" style="width:145px" />
											</div>
