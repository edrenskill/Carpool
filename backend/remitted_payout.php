<?php 
	...
	$_SESSION['fromremitted'] = 1;
	$_SESSION['stat'] = 1;
	IF(isset($_SESSION['fromdrivertripreport'])):
													...
													<table class="table table-striped table-bordered table-hover" id="dataTables-payout">
														...
														<tbody>
																...
																<?php
																	...
																	$records = mysqli_query($link, "SELECT COUNT(A.ID) AS totaltrip, A.driver_ID, A.terminal_ID, A.trip_ID, B.trip_ID, SUM(B.total_fare) AS totalfare, SUM(B.service_fee) AS totalfee, B.bank_ref_no FROM ".DB_PREFIX."vehicle_trip_history A, ".DB_PREFIX."driver_account B WHERE {$conditionset} B.remitted=1 AND B.trip_ID=A.trip_ID AND A.time_date BETWEEN '{$cutoff_start}' AND '{$cutoff_end}' GROUP BY A.driver_ID");
