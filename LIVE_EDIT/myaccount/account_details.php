						...
						<!--<div id="remit" class="col-lg-10">	-->								
							<div class="container-fluid">
								...
								<span style="float:right">Cutoff: <strong  style="color:#0000FF"><?= $daily_cutoff; ?></strong></span>
									...
									$driver = mysqli_fetch_array(mysqli_query($link, "SELECT CONCAT (fname,' ',lname) AS fullname, user_ID FROM ".DB_PREFIX."users WHERE (user_ID='{$driverID1}' OR user_ID='{$driverID2}')"));								
									$DRID = $driver['user_ID'];

									$query = mysqli_query($link, "SELECT status FROM ".DB_PREFIX."account_status WHERE user_ID='{$DRID}' ORDER BY ID DESC LIMIT 1");
									$countquery = mysqli_num_rows($query);
									IF($countquery):
										$account_status = mysqli_fetch_assoc($query);
										$Dstatus = $account_status['status'];
										IF($Dstatus == 0): 
											$status = "<span style='color:#0000ff'>Active</span>";
										ELSEIF($Dstatus == 1):
											$status = "<span style='color:#FF8C00'>Suspended</span>";
										ELSEIF($Dstatus == 2):
