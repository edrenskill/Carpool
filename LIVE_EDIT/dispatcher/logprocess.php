<?php
	...
	if(isset($_POST) && array_key_exists('Login',$_POST)){
		...
		$result = mysqli_query($link, "SELECT `ID`,`pword`,CONCAT(fname,' ',lname) AS full_name,`approval`,`userlevel`,`user_ID`,`terminal_ID` FROM ".DB_PREFIX."users WHERE {$user_cond} AND `account_status` = '0' AND userlevel = 2 AND `ID` = '".$_SESSION['user_id']."'") or die (mysqli_error()); 
