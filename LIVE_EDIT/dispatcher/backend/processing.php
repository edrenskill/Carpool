<?php
	...
	IF(isset($_POST) && array_key_exists('doprocess',$_POST)):
		
		IF(!isset($data['Tstatus']) || $_POST['Tstatus'] == 0):
		
			$err[] = "Please Click the checkbox to Process Application.";	
		
		ELSEIF(isset($data['Tstatus']) && $_POST['Tstatus'] == '1' && empty($err)):

			$date = date("Y-m-d h:i:s");
			$unit_ID = $_SESSION['vehicleID'];
			//Member
		//Terminal
		$process = "UPDATE ".DB_PREFIX."vehicles SET application_status=1 WHERE unit_ID='{$unit_ID}'";

		mysqli_query($link, $process) or die("UPDATE Failed:" . mysqli_error($link));
		
			$successmessage = "process Application";
                ...
                <div class="container">
					...
					<div class="panel panel-default" style="width:350px">
						...
						<?php // Display error message
							...
							<div name="processfield" id="processfield">
								<div>
									<h1 style="color:#1d1d8d"><?= $successmessage; ?></h1>
								</div>
							</div>
							<?php
							...
							<div name="processfield" id="processfield">
								<div>
									<form method="post" name="processing" action="processing" id="processing">
										<fieldset>	
											<div class="form-group"><label>status</label>
                                                <div class="checkbox">
                                                    ...
                                                    <label>
                                                        <input name="Tstatus" id="Tstatus" type="checkbox" value="1"> Processing Application
                                                    </label>
                                                </div>
                                            </div>
											<div class="form-group">
												<button type="submit" class="btn btn-lg btn-success btn-block" value="doprocess" id="doprocess" name="doprocess" >Submit</button>
											</div>
										</fieldset>
									</form>
									</div>
