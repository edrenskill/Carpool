<?php
	...
	IF(isset($_POST) && array_key_exists('doFinalDenied',$_POST)):
		
		IF(!isset($data['Tstatus']) || $_POST['Tstatus'] == 0):
		
			$err[] = "Please Click the checkbox to validate Banning of Member.";	
		
		ELSEIF(isset($data['Tstatus']) && $_POST['Tstatus'] == '5' && empty($err)):

			$date = date("Y-m-d h:i:s");
			$unit_ID = $_SESSION['vehicleID'];
			//Member
		//Terminal
		$denied = "UPDATE ".DB_PREFIX."vehicles SET application_status=5 WHERE unit_ID='{$unit_ID}'";

		mysqli_query($link, $denied) or die("UPDATE Failed:" . mysqli_error($link));
		
			$successmessage = "Final Appealed Application";
                ...
                <div class="container">
					...
					<div class="panel panel-default" style="width:350px">
						...
						<?php // Display error message
							...
							<div name="deniedfield" id="deniedfield">
								<div>
									<form method="post" name="Final_Appealed" action="Final_Appealed" id="Final_Appealed">
										<fieldset>	
											<div class="form-group"><label>Action</label>
                                                <div class="checkbox">
                                                    ...
                                                    <label>
                                                        <input name="Tstatus" id="Tstatus" type="checkbox" value="5">Final Appealed Application
                                                    </label>
                                                </div>
                                            </div>
											<div class="form-group">
												<button type="submit" class="btn btn-lg btn-success btn-block" value="doFinalDenied" id="doFinalDenied" name="doFinalDenied" >Submit</button>
											</div>
										</fieldset>
									</form>
									</div>
