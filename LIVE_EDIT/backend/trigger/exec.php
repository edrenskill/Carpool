		...
		IF($NID_exist):
			...
			IF($card_status['status'] == 0):
				...
				IF($OID_exist):
					...
					mysqli_query($link, "UPDATE `".DB_PREFIX."idcards` SET `status`='1' WHERE `card_number`='{$nID}'");
...
if(isset($data['did']))
{
	...
	$results = mysqli_query($link, "SELECT user_ID FROM ".DB_PREFIX."users WHERE user_ID='{$DID1}' AND userlevel=2 AND banned=0 AND approval=1");
