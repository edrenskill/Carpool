<?php 
	...
	IF(isset($_POST) && array_key_exists('submitUID',$_POST)):
		...
		ELSE:
			...
			$query = mysqli_query($link,"SELECT user_ID FROM `".DB_PREFIX."users` WHERE `user_ID` LIKE '%{$userdata}%' OR `email` LIKE '%{$userdata}%' OR `fname` LIKE '%{$userdata}%' OR `lname` LIKE '%{$userdata}%'");
